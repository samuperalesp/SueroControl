...
export async function sellPackage(id: string, terceroId?: string, medicoId?: string, warehouseId?: string): Promise<any> {
  const res = await apiFetch(`${BASE}/${id}/sell`, {
    method: 'POST',
    body: JSON.stringify({ terceroId, medicoId, warehouseId }),
  });
  if (!res.ok) {
    const msg = await res.text();
